...
| ID       | 기능명               | 설명                                                  | 상태 |
| -------- | -------------------- | ----------------------------------------------------- | ---- |
| **F013** | 관리자 대시보드      | 발행된 견적서 목록 조회 (테이블/카드 반응형 레이아웃) | ✅   |
| **F014** | 클라이언트 링크 복사 | 공개 견적서 URL을 클립보드에 복사 (토스트 피드백)     | ✅   |
| **F015** | 관리자 인증          | 비밀번호 기반 로그인, HMAC-SHA256 세션 쿠키, 미들웨어 | ✅   |
| **F016** | 다크 모드            | light/dark/system 테마 토글 (next-themes)             | ✅   |
...
📱 견적서 시스템
├── 📄 견적서 조회 (공개 접근)
│   └── 기능: F001, F002, F003, F011, F012, F016
├── 🔒 관리자 페이지 (인증 필요)
│   ├── 📋 견적서 목록: F013, F014
│   └── 🔑 로그인: F015
└── 🌙 다크 모드: F016 (모든 페이지)
